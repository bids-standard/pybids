...
class TestSchemaAccuracy:
    ...
    def test_extension_patterns_match_schema(self):
        ...
        schema_extensions = [
            ext['value']
            for ext in bids_schema.objects.extensions.values()
        ...
        coverage = (
            len(matched_extensions) / len(schema_extensions) * 100 if schema_extensions else 0
        )

        error_msg = []
        error_msg.append(
            f'Extension coverage: {coverage:.1f}% ({len(matched_extensions)}/{len(schema_extensions)})'
        )
        if unmatched_extensions:
            error_msg.append(
                f'Unmatched extensions ({len(unmatched_extensions)}): {unmatched_extensions[:20]}'
            )
            if len(unmatched_extensions) > 20:
                error_msg.append(f'... and {len(unmatched_extensions) - 20} more')
        if matched_extensions:
            error_msg.append(
                f'Matched extensions ({len(matched_extensions)}): {matched_extensions[:10]}'
            )
    ...
    def test_suffix_patterns_match_schema(self):
        ...
        schema_suffix_values = [
            obj['value'] for obj in bids_schema.objects.suffixes.values() if obj.get('value')
        ]
        ...
        # Report all differences at once
        coverage = (
            len(matched_suffixes) / len(schema_suffix_values) * 100 if schema_suffix_values else 0
        )

        error_msg = []
        error_msg.append(
            f'Suffix coverage: {coverage:.1f}% ({len(matched_suffixes)}/{len(schema_suffix_values)})'
        )
        if unmatched_suffixes:
            error_msg.append(
                f'Unmatched suffixes ({len(unmatched_suffixes)}): {unmatched_suffixes[:20]}'
            )
            if len(unmatched_suffixes) > 20:
                error_msg.append(f'... and {len(unmatched_suffixes) - 20} more')
        if matched_suffixes:
            error_msg.append(
                f'Matched suffixes ({len(matched_suffixes)}): {matched_suffixes[:10]}'
            )
    ...
    def test_datatype_patterns_match_schema(self):
        ...
        error_msg = []
        error_msg.append(
            f'Datatype coverage: {coverage:.1f}% ({len(matched_datatypes)}/{len(schema_datatypes)})'
        )
        if unmatched_datatypes:
            error_msg.append(
                f'Unmatched datatypes ({len(unmatched_datatypes)}): {unmatched_datatypes}'
            )
