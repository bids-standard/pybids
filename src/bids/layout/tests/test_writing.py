...
import os
import shutil
from os.path import dirname, exists, islink, join
...
from bids.layout.models import Base, BIDSFile, Entity, Tag
from bids.layout.writing import _PATTERN_FIND, build_path
...
{extension<nii|nii.gz|json>|nii.gz}"""
    ...
    def test_build_path(self, writable_file):
        ...
        pats = ['[{session}/]{task}/r-{run}.nii.gz', 't-{task}/{subject}-{run}.nii.gz']
        pats = [join(writable_file.dirname, p) for p in pats]
        target = join(writable_file.dirname, 'rest/r-2.nii.gz')
        assert build_path(writable_file.entities, pats) == target

        # Pattern with conditional values
        pats = ['{task<func|acq>}/r-{run}.nii.gz', 't-{task}/{subject}-{run}.nii.gz']
        pats = [join(writable_file.dirname, p) for p in pats]
        target = join(writable_file.dirname, 't-rest/3-2.nii.gz')
        assert build_path(writable_file.entities, pats) == target

        # Pattern with valid conditional values
        pats = ['{task<func|rest>}/r-{run}.nii.gz', 't-{task}/{subject}-{run}.nii.gz']
        pats = [join(writable_file.dirname, p) for p in pats]
        target = join(writable_file.dirname, 'rest/r-2.nii.gz')
        assert build_path(writable_file.entities, pats) == target
        ...
        with pytest.warns(UserWarning) as record:
            assert (
                build_path({'session': 'B', 'run': 3, 'extension': '.nii'}, pats)
                == 'ses-B/r-3.nii'
            )
        ...
        assert build_path({'session': 'B', 'run': 3, 'extension': '.nii'}, pats) == 'ses-B/r-3.nii'
        ...
        assert sorted(
            build_path(
                {'session': ['A', 'B'], 'run': [1, 2], 'extension': ['.nii.gz', 'json']}, pats
            )
        ) == [
            'ses-A/r-1.json',
            'ses-A/r-1.nii.gz',
    ...
    def test_build_file(self, writable_file, tmp_bids, caplog):
        ...
        with pytest.warns(UserWarning) as record:
            ...
            assert log_message == f'A file at path {target} already exists, skipping writing file.'
...
class TestWritableLayout:
    def test_write_files(self, tmp_bids, layout):
        ...
        example_file = join(
            str(tmpdir), 'sub-02/ses-2/r-1/suffix-bold/acq-fullbrain/task-rest.nii.gz'
        )
        example_file2 = join(
            str(tmpdir), 'sub-01/ses-2/r-1/suffix-bold/acq-fullbrain/task-rest.nii.gz'
        )
        ...
        pat = join(
            str(tmpdir),
            'sub-{subject<01>}/ses-{session}/r-{run}/suffix-{suffix}/task-{task}.nii.gz',
        )
        example_file = join(str(tmpdir), 'sub-01/ses-2/r-1/suffix-bold/task-rest.nii.gz')
    ...
    def test_write_to_file(self, tmp_bids, layout):
        ...
        layout.write_to_file(entities, path_patterns=pat, contents=contents, validate=False)
    ...
    def test_build_file_from_layout(self, tmpdir, layout):
        ...
        pat = join(str(tmpdir), 'sub-{subject}/ses-{session}/r-{run}.nii.gz')
