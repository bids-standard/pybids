...
pl_adj_special_keys = re.compile(rf'^({enclose("|".join(pl_adj_special))})$', re.IGNORECASE)
...
PL_SB_POSTFIX_ADJ_STEMS_RE = re.compile(rf'^(?:{pl_sb_postfix_adj_stems})$', re.IGNORECASE)
PL_SB_PREP_DUAL_COMPOUND_RE = re.compile(rf'^(?:{pl_sb_prep_dual_compound})$', re.IGNORECASE)
...
class engine:
    ...
    def num(self, count: int | None = None, show: int | None = None) -> str:  # (;$count,$show)
    ...
    def _plequal(self, word1: str, word2: str, pl) -> str | bool:  # noqa: C901
    ...
    def _pl_check_plurals_N(self, word1: str, word2: str) -> bool:
        ...
        return (
            ...
            or any(
                self._pl_reg_plurals(pair, stems, end1, end2) for stems, end1, end2 in stem_endings
            )
        )
    ...
    def _pl_general_verb(self, word: str, count: str | int | None = None) -> str:
        count = self.get_count(count)
    ...
    def _pl_special_adjective(self, word: str, count: str | int | None = None) -> str | bool:
        count = self.get_count(count)
    ...
    def ordinal(self, num: int | str) -> str:  # noqa: C901
