...
@click.option(
    ...
    flag_value=True,
    show_default=True,
    type=Either(click.BOOL, click.Path(exists=True)),
...
@click.option('--config', multiple=True, help='Optional name(s) of configuration file(s) to use.')
@click.option(
    '--index-metadata/--no-index-metadata',
    default=False,
    show_default=True,
...
def layout(
    ...
    if reset_db:
        click.echo(f'Successfully generated database index at {db_path}')
    else:
        click.echo(
            f'Previously generated database index found at {db_path}. '
            'To generate a new index, rerun with ``--reset-db``'
        )
...
def upgrade(root):
    ...
    val = click.prompt('Load dataset and update filenames?', default='Y', type=click.Choice('YN'))
    if val == 'Y':
        upgrade_filenames(root, desc)

    if changes:
        click.echo(
            'Upgrade complete. Please run the bids-validator '
...
def upgrade_dataset_description(description):
    ...
    if bidsver is None or bidsver < '1.6.0':
        val = click.prompt(f'Update BIDS Version? (current: {bidsver})', default='1.6.0', type=str)
        if val.startswith('1.'):
    ...
    if 'DatasetType' not in description:
        val = click.prompt(
            'Is this dataset [r]aw or [d]erivative?',
            default='r',
    ...
    if description['DatasetType'] == 'derivative':
        if 'PipelineDescription' in description:
            val = click.prompt(
                'Convert PipelineDescription to GeneratedBy?', default='Y', type=click.Choice('YN')
            )
            if val == 'Y':
...
def upgrade_filenames(root, description):  # noqa: D103
    ...
    with click.progressbar(regressors) as bar:
        for bidsfile in bar:
            ...
            if action is None:
                action = click.prompt(
                    f'Rename {bidsfile.path} to {new_path}? ([y]es/[n]o/[A]ll/[N]one)',
                    default='y',
                    type=click.Choice('ynAN'),
                    show_choices=False,
                )
                if action in 'AN':
                    policy = action
...
@click.option(
    '--sub_label', '-s', multiple=True, help='The label(s) of the subjects(s) to create report for'
)
@click.option(
    '--ses_label', '-ss', multiple=True, help='The label(s) of the session(s) to create report for'
)
@click.option(
    '--task_label', '-t', multiple=True, help='The label(s) of the task(s) to create report for'
)
@click.option(
    '--run_label', '-r', multiple=True, help='The label(s) of the run(s) to create report for'
)
def model_report(
    ...
    generate_report(model, root, derivatives, output_dir, scan_length, **entities)
