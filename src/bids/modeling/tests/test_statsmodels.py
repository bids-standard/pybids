...
def test_repr(graph):
    assert (
        graph.__repr__()
        == "<BIDSStatsModelsGraph[{name='test_model', description='simple test model', ... }]>"
    )
...
def test_entire_graph_smoketest(graph):
    ...
    out_contrasts = [c.entities['contrast'] for c in cis if c.entities['subject'] == '01']

    expected_outs = ['gain', 'gain_neg', 'RT', 'RT_neg', 'RT:gain', 'RT:gain_neg']
...
def test_expand_wildcards():
    ...
    # Some examples
    assert expand_wildcards(['trial_type.*'], ['trial_type.A', 'trial_type.B']) == [
        'trial_type.A',
        'trial_type.B',
    ]
